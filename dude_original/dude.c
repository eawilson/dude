...
int main (int argc, char **argv) {
    ...
    while (c != -1) {
        ...
        switch (c) {
            case 'a':
                allowed = arg_to_int(optarg);
                break;
                    
            case 'f':
                min_family_size = arg_to_int(optarg);
                break;
    ...
    if (argc - optind != 2) {
        fprintf(stderr, "Filenames of exactly two paired FASTQ files are required arguments.\n");
        exit(-1);
        }
    read1 = argv[optind++];
    read2 = argv[optind++];
...
static PairedFastq read_fastqs(char *read1, char *read2) { 
    ...
    for (read = R1; read <= R2; read++) {
        if (strcmp(filenames[read] + strlen(filenames[read]) - 6 , ".fastq") != 0) {
            fprintf(stderr, "Not a fastq.");
            goto cleanup;
            }        
        fp[read] = fopen(filenames[read], "r");
        if (fp[read] == NULL) {
            fprintf(stderr, "Unable to open fastq.");
            goto cleanup;
            }
    ...
    for (readpair = fq.readpairs; readpair < fq.readpairs + fq.total_reads; readpair++) {
        for (read = R1; read <= R2; read++) {
            ...
            if (bytes_read != readpair->read[read].len) {
                fprintf(stderr, "Sequence and quality differ in length.");
                goto cleanup;
                }
            }
        ...
        for (j = 0; !(readpair->read[R1].name[j] == '\0' && readpair->read[R2].name[j] == '\0'); j++) {
            if (readpair->read[R1].name[j] != readpair->read[R2].name[j] && !(readpair->read[R1].name[j] == '1' && readpair->read[R2].name[j] == '2')) {
                fprintf(stderr, "Reads 1 and 2 names dont match.");
                goto cleanup;
                }
            }
...
static int size_and_remove_umis(PairedFastq *fq, int allowed, int thruplex) {
    ...
    for (readpair = fq->readpairs; readpair < fq->readpairs + fq->total_reads; readpair++) {
        ...
        n += do_they_overlap(readpair, 30, allowed);
...
static int remove_unconfirmed_reads(PairedFastq *fq, int min_family_size) {
    ...
    readpair2 = fq->readpairs;
    for (readpair = fq->readpairs; readpair < fq->readpairs + fq->total_reads; readpair++) {
        if (readpair->copy_number >= min_family_size && (readpair->fragment_size > 0 || min_family_size != 1)) {
            if (readpair > readpair2)
                *readpair2 = *readpair;
            readpair2++;
            }
